...
# Fallback clearweb search engines (proxies for dark web)
FALLBACK_SEARCH_ENGINES = [
    {"name": "Ahmia (Clearweb)", "url": "https://ahmia.fi/search/?q={query}"},
]
...
def fetch_search_results(endpoint, query, use_tor=True):
        ...
        if response.status_code == 200:
            ...
            for a in soup.find_all('a'):
                try:
                    ...
                    # Extract onion links (even from clearweb gateways)
                    link_match = re.findall(r'https?:\/\/[a-z0-9\.]+\.onion.*', href)
                    if not link_match:
                        # Sometimes links are relative or encoded in Ahmia
                        if "ahmia.fi" in url and "/redirect/" in href:
                            encoded_link = href.split("redirect?search_result=")[-1]
                            # Simple cleanup for Ahmia redirects
                            if ".onion" in encoded_link:
                                link_match = [encoded_link]
                    
                    if link_match:
                        # Basic filtering to avoid self-referential links
                        if "search" not in link_match[0] and len(title) > 3:
                            links.append({"title": title, "link": link_match[0]})
                except:
                    continue
            return links
        return []
...
def get_search_results(refined_query, max_workers=5):
    ...
    print("[*] Attempting deep dark web search (this may take a minute)...")
    ...
    # If no results found via Tor, try clearweb fallbacks
    if not results:
        print("[!] Onion search failed or returned no results. Trying clearweb fallbacks...")
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(fetch_search_results, e["url"], refined_query, use_tor=False)
                       for e in FALLBACK_SEARCH_ENGINES]
            for future in as_completed(futures):
                result_urls = future.result()
                if result_urls:
                    results.extend(result_urls)
