<div align="center">
   ...
   <br><a href="https://github.com/apurvsinghgautam/dcrawler/actions/workflows/release.yml"><img alt="Release" src="https://github.com/apurvsinghgautam/dcrawler/actions/workflows/release.yml/badge.svg"></a> <a href="https://github.com/apurvsinghgautam/dcrawler/releases"><img alt="GitHub Release" src="https://img.shields.io/github/v/release/apurvsinghgautam/dcrawler"></a> <a href="https://hub.docker.com/r/apurvsg/dcrawler"><img alt="Docker Pulls" src="https://img.shields.io/docker/pulls/apurvsg/dcrawler"></a>
   <h1>Dcrawler: AI-Powered Dark Web OSINT Tool</h1>

   <p>Dcrawler is an AI-powered tool for conducting dark web OSINT investigations. It leverages LLMs to refine queries, filter search results from dark web search engines, and provide an investigation summary.</p>
...
- 💻 **CLI Interface** – Powerful command-line tool for fast investigations.
- 🐳 **Docker-Ready** – Recommended Docker deployment for clean, isolated usage.
- 📝 **Custom Reporting** – Intelligence summaries printed directly to your terminal.
...
>
> Additionally, Dcrawler leverages third-party APIs (including LLMs). Be cautious when sending potentially sensitive queries, and review the terms of service for any API or model provider you use.
...
- Pull the latest Dcrawler docker image
```bash
docker pull apurvsg/dcrawler:latest
```
...
docker run --rm \
   ...
   apurvsg/dcrawler:latest "your search query"
```
...
python3 dcrawler.py "your search query"
```

## Usage

```bash
python3 dcrawler.py [query] [options]
```

**Options:**
- `-m`, `--model`: Select LLM model (e.g., `gemini-2.5-flash`, `gpt-4o`).
- `-t`, `--threads`: Number of concurrent scraping/search threads (default: 4).
- `--max-results`: Maximum raw search results to consider (default: 50).
- `--max-scrape`: Maximum filtered results to scrape for content (default: 10).
